...
try:
    ...
    writeConfigDir(home) # this will create the configdir text file and store the dir based on what the user chooses
    with open(home+"EOLT-Test-Analyzer-configdir.txt") as f:
        configdir = f.read()
...
if mode == "d":
    try:
        ...
        if type(dirs) != list:
            dirs = [dirs] # if dirs is saved as a string, then convert it to a list 
    ...
    outdir = filedialog.askdirectory(title="Select the output directory") + "/" # ask for outdir
...
def createFile():
    ...
    with open(
        ...
        locs["out_dir"] = outdir + \
            ("/" if outdir[-1] != "/" and outdir[-1] != "\\" else "") if outdir!="" else "" # add a "/" to the end of outdir if it doesn't already have one
        locs["certificate_dir"] = certdir + (
            ("/" if certdir[-1] != "/" and certdir[-1] != "\\" else "") if certdir!="" else "" # add a "/" to the end of certdir if it doesn't already have one
        )
        locs["search_dirs"] = [
